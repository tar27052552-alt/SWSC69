...
const startStr = '      // Filter by category and sort by date descending (approximate using id if date parsing fails)';
const endStr = '      `).join(\'\');';
...
if (startIdx !== -1) {
    ...
    if (endIdx !== -1) {
        ...
        const newCode = `      // Filter by category and sort by date descending (approximate using id if date parsing fails)
      ...
      if (filtered.length === 0) {
        body.innerHTML = '<div style="text-align:center; padding:30px 20px; color:var(--gray-500);">ยังไม่มีเอกสารในหมวดหมู่นี้</div>';
        return;
      }
